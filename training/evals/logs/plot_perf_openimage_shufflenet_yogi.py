...
def main(files):
    ...
    for index,file in enumerate(files):
        ...
        if index==1:
            metrics[-1]=metrics[-1][:min(33,len(metrics[-1]))]
        if index==0:
            metrics[-1]=metrics[-1][:min(25,len(metrics[-1]))]
    ...
    plot_line(metrics, walltime, setting_labels, 'Training Time (hour)', plot_metric, 'time_to_acc_openimage_shufflenet_yogi.png')
